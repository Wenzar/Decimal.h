...
START_TEST(test_div_manual3) {
  ...
  float value1_float = s21_rand_r(-1e20, 1e20);
  float value2_float = s21_rand_r(-1e20, 1e20);
  ...
  int return_value = s21_div(value_1, value_2, &result);
  s21_from_decimal_to_float(result, &check_float);
  ck_assert_float_eq_tol(check_float, value1_float / value2_float, 1e-6);
  ck_assert_int_eq(return_value, 0);
}
END_TEST
...
Suite *s21_div_suite(void) {
  ...
  tcase_add_loop_test(test_case, test_div_manual3, 0, 100);
