...
int s21_div(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  int add_return = 0;
  if(result!=NULL){
    if(error_scale(value_1)||error_scale(value_2)||small_find_out_the_degree(value_1)>28||small_find_out_the_degree(value_2)>28){
      add_return=3;
    }
    else{
    ...
    if ((!big_getBit(current_value_1, 223) &&
      ...
      if (mantis_s21_is_equal(current_value_1, current_value_2)) {
        ...
        while (find_out_the_degree(current_value_1) != 28 &&
               !raise_scale(&tmp_decimal)&&mantis_is_greater(current_value_2,current_value_1)) {
