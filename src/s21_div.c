...
int s21_div(s21_decimal value_1, s21_decimal value_2, s21_decimal *result)
{
    ...
    if ((!big_getBit(current_value_1, 223) && !big_getBit(current_value_2, 223)) || (big_getBit(current_value_1, 223) && big_getBit(current_value_2, 223)))
    {
        ...
        else
        {
            ...
            while (find_out_the_degree(current_value_1) != 28 && !raise_scale(&tmp_decimal))
            {
    ...
    else if ((!big_getBit(current_value_1, 223) && big_getBit(current_value_2, 223)) || (big_getBit(current_value_1, 223) && !big_getBit(current_value_2, 223)))
    {
        if (mantis_s21_is_equal(current_value_1, current_value_2))
        {
            big_result = one_big_decimal;
